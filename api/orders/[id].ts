...
export default function handler(request: Request, response: Response) {
  ...
  const rawId = Array.isArray(idParameter) ? idParameter[0] : idParameter;
  ...
  const order = orders.find((item) => {
    ...
    const cleanedOrderId = originalOrderId.replace(/[^A-Z0-9]/g, "");

    return originalOrderId === normalizedId || cleanedOrderId === cleanId;
  });
